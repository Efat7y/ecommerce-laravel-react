...
pattern = r'(<div className="flex items-center gap-2">[^<]*<span[^>]*>[^<]*</span>[^<]*<input[^>]*value=\{sp\.discount_price\}[^>]*onChange=\{\(e\) =>\s*updateSaleProductPrice\(\s*sp\.product_id,\s*e\.target\.value\s*\)\s*\}[^>]*className="w-20 p-1 border rounded text-center dark:bg-gray-600"[^>]*/>[^<]*</div>)'
...
print("Quantity UI injected via precise regex")
